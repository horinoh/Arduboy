...
public:
	...
	static Vec3<T> Zero() { return { 0.0, 0.0, 0.0 }; }
	static Vec3<T> AxisX() { return { 1.0, 0.0, 0.0 }; }
	static Vec3<T> AxisY() { return { 0.0, 1.0, 0.0 }; }
	static Vec3<T> AxisZ() { return { 0.0, 0.0, 1.0 }; }
...
template<typename T> Vec3<T> operator*(const Vec3<T>& lhs, const T& rhs) {
	using namespace Component;
	return { lhs[X] * rhs, lhs[Y] * rhs, lhs[Z] * rhs };
}
template<typename T> Vec3<T> operator*(const T& lhs, const Vec3<T>& rhs) { rhs* lhs; }
template<typename T> Vec3<T> operator/(const Vec3<T>& lhs, const T& rhs) { return lhs * (T(1.0) / rhs); }
